...
void cancelSystemCall(globalState& gs, state& s, ptracer& t) {
  ...
  // Run the (now skipped) system call to its syscall-exit stop. Do not
  // single-step: on powerpc a single-step out of a seccomp stop does not
  // trap after one instruction but runs on to the next ptrace event
  // (e.g. the fork event of glibc's fallback clone), which this waitpid
  // would then silently swallow.
  ptracer::doPtrace(PTRACE_SYSCALL, pid, 0, 0);
